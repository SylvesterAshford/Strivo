...
import type { StrategicInsights, Recommendation, BusinessMetrics, Swot } from "@/lib/api";
    ...
    <View style={styles.card}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: spacing.lg }}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
          <Icon name={trendUp ? "trending-up" : "trending-down"} size={14} color={trendUp ? colors.semantic.positive : colors.semantic.critical} />
          <AppText variant="caption" style={{ color: trendUp ? colors.semantic.positive : colors.semantic.critical }}>
        ...
        {series.map((d, i) => {
          ...
          return (
            <View key={i} style={styles.chartCol}>
              <View className="bar-enter" style={[styles.bar, { height: h }]} />
            </View>
          );
        })}
      </View>
      ...
      <View style={styles.metricRow}>
        ...
        <Metric label="အမြတ်" value={formatCurrency(metrics.netProfitMmk)} color={metrics.netProfitMmk >= 0 ? colors.semantic.positive : colors.semantic.critical} />
      ...
      <View style={styles.chartRow}>
        {actual.map((v, i) => (
          <View key={`a${i}`} style={styles.chartCol}>
            <View className="bar-enter" style={[styles.bar, { height: Math.max(4, (v / max) * 56), backgroundColor: colors.text.tertiary }]} />
          </View>
        ))}
        {forecast.map((v, i) => (
          <View key={`f${i}`} style={styles.chartCol}>
            <View className="bar-enter" style={[styles.bar, { height: Math.max(4, (v / max) * 56), backgroundColor: colors.accent.base }]} />
          </View>
        ))}
      </View>
    ...
    <View style={styles.card}>
      ...
      {segs.map((s, i) => (
        <View key={s.key} style={{ marginBottom: i < segs.length - 1 ? spacing.md : 0 }}>
          ...
          <View style={styles.scoreTrack}>
            <View style={[styles.scoreFill, { width: `${(s.totalMmk / max) * 100}%`, backgroundColor: palette[i] }]} />
          </View>
        </View>
      ))}
    </View>
