...
import { View, Pressable, StyleSheet, KeyboardAvoidingView, Platform, TextInput, Image, ActivityIndicator, Alert, useSafeAreaInsets } from "@/rn";
...
export default function LoginScreen() {
  ...
  const requireConfigured = () => {
    if (!isSupabaseConfigured()) {
      setError("Supabase not configured — add creds to .env.local");
      return false;
    }
    return true;
  };
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: colors.bg.base }}>
      <View
        style={{
          flex: 1,
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <AppText variant="body" color="secondary" style={{ textAlign: "center", marginTop: spacing.xs, marginBottom: spacing.lg }}>
            {my.auth.subtitle}
          </AppText>
          ...
          <Pressable
            onPress={onGoogle}
            disabled={busy !== null}
            style={({ pressed }) => [styles.googleBtn, { opacity: busy !== null ? 0.6 : pressed ? 0.85 : 1, transform: [{ scale: pressed ? 0.99 : 1 }] }]}
          >
            {busy === "google" ? (
          ...
          <View style={styles.passwordLabelRow}>
            ...
            <Pressable onPress={onForgotPassword} disabled={busy !== null}>
          ...
          {error ? (
            <AppText variant="caption" style={{ color: colors.semantic.critical, textAlign: "center", marginTop: spacing.md }}>
              {error}
            </AppText>
          ) : null}
        </View>
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    padding: spacing["2xl"],
    boxShadow: `0 8px 24px ${colors.accent.glow}`,
  },
  iconCircle: {
    alignSelf: "center",
    width: 40,
