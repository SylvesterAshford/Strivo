...
export default function RivalDetailsStep() {
  ...
  const [rows, setRows] = useState<Row[]>(() =>
    names.map((name) => {
      ...
      return { name, tier: existing?.tier ?? "matcher", audience: existing?.audience ?? "" };
    }),
  );
  ...
  const onNext = () => {
    const details: RivalDetail[] = rows.map((r) => ({ name: r.name, tier: r.tier, audience: r.audience.trim() }));
        ...
        {rows.map((r, i) => (
          ...
          >
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
              {TIERS.map((t) => (
                <Chip key={t.value} label={t.label} selected={r.tier === t.value} onPress={() => setRow(i, { tier: t.value })} />
              ))}
            </View>
            <TextField value={r.audience} onChangeText={(v) => setRow(i, { audience: v })} placeholder={my.onboarding.audiencePlaceholder} />
          </View>
        ))}
      </View>
    </WizardStep>
