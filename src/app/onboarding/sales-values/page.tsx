...
export default function SalesValuesStep() {
  ...
  const [values, setValues] = useState<Partial<Record<SalesPeriod, string>>>(() =>
    Object.fromEntries(Object.entries(draft.salesValues).map(([k, v]) => [k, String(v)])),
  );
      ...
      <View style={{ gap: spacing.lg }}>
        {periods.map((p) => (
          <View key={p} style={{ gap: spacing.xs }}>
            ...
            <TextField value={values[p] ?? ""} onChangeText={(v) => setVal(p, v)} keyboardType="number-pad" placeholder="0" />
          </View>
        ))}
      </View>
    </WizardStep>
