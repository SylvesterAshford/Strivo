...
import { useLocalSearchParams, useRouter } from "@/rn/router";
...
function ConfirmFactsInner() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const queryClient = useQueryClient();
  const params = useLocalSearchParams<{ recordingId: string; transcript: string; facts: string }>();
      ...
      <View style={[styles.root, { paddingTop: insets.top + spacing.lg }]}>
        ...
        <ScrollView style={styles.scrollOuter} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <Pressable onPress={() => setShowTranscript((v) => !v)} style={styles.transcriptToggle}>
            ...
            <Icon name={showTranscript ? "chevron-up" : "chevron-down"} size={14} color={colors.text.secondary} />
          </Pressable>
          ...
          <View style={{ marginTop: spacing["3xl"] }}>
            ...
            {facts.length === 0 ? (
              ...
              facts.map((f, i) => <FactCard key={i} fact={f} index={i} onChange={handleChange} onRemove={handleRemove} />)
            )}
          </View>
        ...
        <View style={[styles.footer, { paddingBottom: insets.bottom + spacing.lg }]}>
          <Pressable onPress={() => router.replace("/")} style={styles.discardBtn}>
          ...
          >
            {saving ? <ActivityIndicator color={colors.text.onDark} /> : <AppText variant="bodyMedium" color="onDark">{my.common.saveAll}</AppText>}
          </Pressable>
        </View>
      </View>
