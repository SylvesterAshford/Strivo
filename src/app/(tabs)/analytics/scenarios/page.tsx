...
import { View, StyleSheet, ActivityIndicator, Pressable } from "@/rn";
...
export default function ScenariosScreen() {
  ...
  return (
    <Screen>
      ...
      {error ? <ErrorBanner error={error} onRetry={() => run(error.retry)} /> : null}

      <View style={{ marginTop: spacing.lg }}>
        <Button label={my.scenarios.runCta} onPress={() => run(text)} disabled={text.trim().length < 5} />
      </View>
    </Screen>
  );
}
...
function ImpactTile({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <View style={styles.impactTile}>
      ...
      <AppText variant="title" style={{ color, textAlign: "center", marginTop: spacing.sm, flexShrink: 1 }} numberOfLines={2}>
        {value}
      </AppText>
    </View>
  );
