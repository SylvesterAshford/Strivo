...
import {
  importExpensesPreview,
  importExpensesConfirm,
  importExpensesText,
  type ExpenseImportPreviewResponse,
  type ExpenseColumnMapping,
} from "@/lib/api";
...
export default function ImportExpensesScreen() {
  ...
  const onPickFile = async () => {
    ...
    setBusy(true);
    try {
      const data = await importExpensesPreview(
        asset.uri,
        asset.name,
    ...
    return (
      <Screen>
        ...
        <View style={{ marginTop: spacing["2xl"], gap: spacing.md }}>
          <Button
            label={busy ? "..." : my.importExpenses.confirmImport}
  ...
  if (mode === "paste") {
    return (
      <Screen>
        ...
        <View style={{ marginTop: spacing["2xl"], gap: spacing.md }}>
          <Button
            label={busy ? my.importExpenses.analyzing : my.importExpenses.importAndFinish}
  ...
  return (
    <Screen>
      ...
      <View style={{ gap: spacing.md }}>
        <Button
          label={busy ? my.importExpenses.analyzing : my.importExpenses.pickFile}
          onPress={onPickFile}
          disabled={busy}
        />
        <Button
          label={my.importExpenses.pasteCta}
          variant="secondary"
  ...
  return (
    <View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: spacing.sm }}>
        <RoleChip label={my.importExpenses.roleNone} selected={selectedIdx === -1} onPress={() => onPick(-1)} />
        {headers.map((h, i) => (
