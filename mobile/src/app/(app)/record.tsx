...
// The center dock action: a hub for getting data into the app. Voice capture
// was removed; this now routes to the bulk import flows and manual entry.
interface Option {
  icon: IconName;
  title: string;
  subtitle: string;
...
const IMPORTS: Option[] = [
  {
    icon: "spreadsheet",
    title: my.addData.salesTitle,
    subtitle: my.addData.salesSubtitle,
    route: "/import-sales",
    tint: colors.chart.plum,
  },
  {
    icon: "tag",
    title: my.addData.expensesTitle,
    subtitle: my.addData.expensesSubtitle,
    route: "/import-expenses",
    tint: colors.chart.terracotta,
  },
  {
    icon: "package",
    title: my.addData.productsTitle,
    subtitle: my.addData.productsSubtitle,
    route: "/import-products",
    tint: colors.chart.dustyBlue,
  },
];
    ...
    <View style={[styles.root, { paddingTop: insets.top + spacing.lg }]}>
      ...
      >
        ...
        <Card
          opt={{
            icon: "pencil",
            title: my.addData.manualTitle,
            subtitle: my.addData.manualSubtitle,
            route: "/manual-entry",
...
function Card({ opt, onPress }: { opt: Option; onPress: () => void }) {
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [styles.card, pressed && { opacity: 0.7, transform: [{ scale: 0.99 }] }]}
    >
