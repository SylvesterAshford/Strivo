...
export default function ManualEntryScreen() {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const amountMmk = showAmount && amount.trim()
        ? parseInt(amount.replace(/[^0-9]/g, ""), 10) || undefined
        : undefined;
    ...
    >
      <View style={[styles.root, { paddingTop: insets.top + spacing.lg }]}>
        ...
        >
          ...
          {/* Description */}
          <Eyebrow style={{ marginTop: spacing["3xl"], marginBottom: spacing.sm }}>
            {my.manualEntry.descLabel}
          </Eyebrow>
          ...
          {/* Counterparty */}
          <Eyebrow style={{ marginTop: spacing["2xl"], marginBottom: spacing.sm }}>
            {my.manualEntry.counterpartyLabel}
          </Eyebrow>
          <View style={styles.inputBox}>
            <TextInput
              value={counterparty}
        ...
        <View style={[styles.footer, { paddingBottom: insets.bottom + spacing.lg }]}>
          ...
          >
            <AppText variant="body" color="secondary">{my.common.cancel}</AppText>
          </Pressable>
          ...
          <Pressable
            ...
            style={({ pressed }) => [
              styles.saveBtn,
              { transform: [{ scale: pressed ? 0.97 : 1 }], opacity: saving || !canSave ? 0.5 : 1 },
            ]}
          >
            {saving ? (
              <ActivityIndicator color={colors.text.onDark} />
            ) : (
              <AppText variant="bodyMedium" color="onDark">{my.common.save}</AppText>
            )}
          </Pressable>
        </View>
      </View>
